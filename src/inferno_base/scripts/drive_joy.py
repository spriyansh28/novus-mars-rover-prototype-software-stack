...
class DriveTeleop:
    ...
    def on_joy(self, data):
        ...
        if data.axes[1] == 1: # full speed (d-pad up)
            self.speed_setting = 1
        if data.axes[0] != 0: # medium speed (d-pad left or right)
            self.speed_setting = 2
        if data.axes[1] == -1: # low speed (d-pad down)
            self.speed_setting = 3

        # Drive sticks
        left_speed = data.axes[1] #/ self.speed_setting # left stick
        right_speed = -data.axes[0] #/ self.speed_setting # right stick
